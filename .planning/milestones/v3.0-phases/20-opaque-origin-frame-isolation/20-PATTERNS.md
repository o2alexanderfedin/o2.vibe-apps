...
**Note on "iframe"/"sandbox"/"isolation" ban (HYGIENE-07 — concrete gate, see Plan 05 Task 3):** These words must not appear in any user-visible copy or devtools-visible surface, but they ARE legitimate internal identifiers (`<iframe>`, `SandboxFrame`, `frameBridge`/`frameMount`, `import ... "./frameMount"`). The existing `BANNED` array (lines 46-53) is pure per-line regex — there is NO context-aware/AST scanner here to reuse, and the D-40 "PHASE 2 RELAXATION" note is aspirational, not implemented. So do NOT add `/iframe|sandbox|isolation/` to the repo-wide `BANNED` set (it would false-positive on the new files' own JSX/identifiers), and do NOT build a new AST/JSX extractor. Instead enforce with a SEPARATE focused line-based check (the simpler, robust path) that flags `\b(iframe|sandbox|isolation)\b` ONLY in: (1) `.css` and `.html` files repo-wide (entirely devtools-visible surfaces), and (2) within `.ts`/`.tsx`, ONLY inside quoted string literals or comments — the same per-line granularity the existing `generate` family already uses — so code outside quotes/comments (identifiers, JSX element/attribute names, import specifiers) is never flagged. Carve out the one legitimate quoted use, `sandbox="allow-scripts"`, by stripping that exact token per line before matching (the surgical way `DEPENDENCY_ALLOWLIST` strips `fake-indexeddb`). This satisfies HYGIENE-07 while letting `<iframe sandbox="allow-scripts">`, `SandboxFrame`, and the module imports pass — and would correctly fail `aria-label="isolation frame"` or `<p>This runs in a sandbox</p>`.
